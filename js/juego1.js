...
window.onload = () => {
    ...
    if (document.getElementById("pantalla-carga-overlay")) {
        window.addEventListener("pantallaCargaTerminada", () => {
            hablar("Juego uno. Suma de Diamantes. Arrastra cada operacion de suma al cofre que tenga el resultado correcto.");
        });
    } else {
        hablar("Juego uno. Suma de Diamantes. Arrastra cada operacion de suma al cofre que tenga el resultado correcto.");
    }
...
function configurarHablarElementos() {
    ...
    document.querySelectorAll(".draggable-card").forEach(card => {
        card.addEventListener("click", () => {
            ...
            // Cambiar símbolos para mejor lectura por voz
            const textoVoz = op.replace("+", "más");
            hablar(textoVoz);
        });
    });
...
function configurarDragAndDrop() {
    ...
    cards.forEach(card => {
        card.addEventListener("dragstart", (e) => {
            ...
            const op = card.getAttribute("data-op");
            hablar("Arrastrando " + op.replace("+", "más"));
            sndClick.play().catch(() => {});
        });
    });
...
function completarNivel() {
    ...
    if (progresoActual < 1) {
        localStorage.setItem("progresoJuegos", "1");
    ...
    hablar("¡Excelente! Completaste el Juego uno de Sumas. Presiona siguiente para avanzar al Juego dos.");
